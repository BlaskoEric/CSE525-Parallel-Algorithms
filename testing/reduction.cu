 ...
 * an array, which will reduce the overall size that needs to be 
 * worked on each iteration. Once the reduction is complete, the 
 * final sum of all values will be displayed from array[0]. 
 *
 ...
 * then sync with other threads. Each thread will perform the 
 * reduction until only the first index has the partial sum. The 
 * partial sum will then be stored in the d_array based on the Block 
 * index number. This makes sure that all partial sums are to the 
 * front of the array.
 ********************************************************************/
